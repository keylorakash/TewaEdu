...
if (!fs.existsSync(INTAKES_FILE)) {
  const defaultIntakes = {
    april: { id: 'april', icon: '🌸', title: 'April Intake', subtitle: 'Main Academic Year Start', applicationDeadline: 'Nov 30, 2025', courseDuration: '1-2 Years', visaProcessing: 'Dec 2025 - Jan 2026', departure: 'Late March 2026', scholarship: '100% Available', scholarshipColor: 'red', partTimeWork: '28 hrs/week', buttonText: 'Apply for April Intake →' },
    october: { id: 'october', icon: '🍂', title: 'October Intake', subtitle: 'Second Major Intake', applicationDeadline: 'May 31, 2026', courseDuration: '1.5 Years', visaProcessing: 'Jun - Jul 2026', departure: 'September 2026', scholarship: 'Partial to Full', scholarshipColor: 'blue', partTimeWork: '28 hrs/week', buttonText: 'Apply for October Intake →' }
  };
...
if (!fs.existsSync(DASHBOARD_FILE)) {
  fs.writeFileSync(DASHBOARD_FILE, JSON.stringify({
    ...
    profile: { name: 'TEWA Education', role: 'Japan Education Consultancy', bio: 'Helping students study, work, and build their future in Japan.', email: 'tewa.educ@gmail.com', phone: '+977-9767474000', image: '/images/logo.png' },
    photos: [{ image: '/images/logo.png', caption: 'TEWA Education logo' }],
    containers: [
      { id: 'home', name: 'Hero', heading: 'Study In Japan' },
      { id: 'about', name: 'About TEWA', heading: 'Guiding Your Path to Success in Japan' },
      { id: 'language-programs', name: 'Language Programs', heading: 'Japanese Language Courses & Tests' },
      { id: 'kaigo-program', name: 'Kaigo Program', heading: 'Kaigo (介護) Scholarship Program' },
      ...
      { id: 'testimonials', name: 'Testimonials', heading: 'Success Stories' },
    ...
    whyChoose: { subtitle: 'Your trusted partner for Japan education since 2024', items: [
      { icon: '📅', title: 'Flexible Class Schedule', text: 'Learn Japanese at timings that fit your lifestyle and commitments.' },
      { icon: '👨‍🏫', title: 'Experienced Instructors', text: 'Practical, engaging, and result-oriented Japanese language training.' },
      { icon: '📋', title: 'Complete Application Support', text: 'From school selection to document preparation, we simplify the process.' },
      { icon: '📝', title: 'Exam Registration Assistance', text: 'Get professional guidance for Japanese language proficiency tests.' },
      { icon: '🎓', title: 'Scholarship Opportunities', text: 'Explore special scholarship pathways, including Kaigo (介護) programs, one of the most in-demand jobs in Japan.' }
    ]}
  }, null, 2));
}
...
function ensureAdminAuth() {
  if (!fs.existsSync(ADMIN_AUTH_FILE)) {
    ...
    fs.writeFileSync(ADMIN_AUTH_FILE, JSON.stringify(hashPassword(initialPassword), null, 2));
  }
}
...
function resolveIntakeId(id) {
  const aliases = { april2026: 'april', 'april-2026': 'april', april2027: 'april', 'april-2027': 'april', october2026: 'october', 'october-2026': 'october' };
...
function requireAdmin(req, res, next) {
  ...
  if (!expiresAt || expiresAt <= Date.now()) {
    ...
    return res.status(401).json({ error: 'Please log in as an administrator.' });
  }
  next();
}
...
app.get('/api/dashboard', (req, res) => {
  try {
    ...
    const applications = JSON.parse(fs.readFileSync(APPLICATIONS_FILE, 'utf-8'));
    const messages = JSON.parse(fs.readFileSync(MESSAGES_FILE, 'utf-8'));
    const cardCounts = {
      Applications: applications.length,
      'New Leads': applications.filter((application) => application.status === 'New Lead').length,
...
app.post('/api/applications', (req, res) => {
  const requiredFields = ['fullName', 'phone', 'email', 'intake', 'interest', 'education'];
  const missingField = requiredFields.find((field) => typeof req.body?.[field] !== 'string' || !req.body[field].trim());
...
const testimonialUpload = multer({
  ...
  fileFilter: (req, file, callback) => {
    ...
    const validType = ['image/jpeg', 'image/png', 'image/webp'].includes(file.mimetype);
    callback(null, validType && ['.jpg', '.jpeg', '.png', '.webp'].includes(extension));
  }
});
...
function validateTestimonial(body) {
  ...
  return requiredFields.find((field) => typeof body?.[field] !== 'string' || !body[field].trim());
}

app.post('/api/testimonials', requireAdmin, testimonialUpload.single('image'), (req, res) => {
  const missingField = validateTestimonial(req.body);
  if (missingField) return res.status(400).json({ error: `${missingField} is required.` });
  if (!req.file) return res.status(400).json({ error: 'Student image is required.' });

  try {
    const testimonials = JSON.parse(fs.readFileSync(TESTIMONIALS_FILE, 'utf-8'));
    const testimonial = { id: crypto.randomUUID(), name: req.body.name.trim(), quote: req.body.quote.trim(), details: req.body.details.trim(), image: `/uploads/testimonials/${req.file.filename}` };
...
app.put('/api/testimonials/:id', requireAdmin, testimonialUpload.single('image'), (req, res) => {
  const missingField = validateTestimonial(req.body);
  if (missingField) return res.status(400).json({ error: `${missingField} is required.` });

  try {
    const testimonials = JSON.parse(fs.readFileSync(TESTIMONIALS_FILE, 'utf-8'));
    const testimonialIndex = testimonials.findIndex((testimonial) => testimonial.id === req.params.id);
    if (testimonialIndex < 0) return res.status(404).json({ error: 'Success story not found.' });
    const previousImage = testimonials[testimonialIndex].image;
    testimonials[testimonialIndex] = { id: req.params.id, name: req.body.name.trim(), quote: req.body.quote.trim(), details: req.body.details.trim(), image: req.file ? `/uploads/testimonials/${req.file.filename}` : previousImage };
...
function findMessageIndex(messages, messageId) {
  return messages.findIndex((message) => message.id === messageId || message.timestamp === messageId);
}
...
app.put('/api/messages/:id', requireAdmin, (req, res) => {
  try {
    ...
    if (typeof req.body?.read !== 'boolean') return res.status(400).json({ error: 'Read status must be true or false.' });
...
app.put('/api/dashboard', requireAdmin, (req, res) => {
  try {
    ...
    if (!Array.isArray(nextDashboard.cards) || nextDashboard.cards.length !== 6) {
      return res.status(400).json({ error: 'Dashboard must contain exactly six cards.' });
    }
...
app.put('/api/intakes/:id', requireAdmin, (req, res) => {
  try {
    ...
    if (!intakes[intakeId]) return res.status(404).json({ error: 'Intake not found' });
    intakes[intakeId] = { ...intakes[intakeId], ...req.body };
    fs.writeFileSync(INTAKES_FILE, JSON.stringify(intakes, null, 2));
    res.json({ success: true, message: 'Intake updated successfully', data: intakes[intakeId] });
  } catch (error) {
    res.status(500).json({ error: 'Failed to update intake data' });
  }
});

app.get('/api/health', (req, res) => res.json({ status: 'Server is running', timestamp: new Date() }));
...
app.listen(PORT, () => {
  ...
  console.log(`   PUT  http://localhost:${PORT}/api/intakes/:id (requires login)\n`);
});
