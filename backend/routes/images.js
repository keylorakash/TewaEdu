...
function ensureImagesFile() {
  if (!fs.existsSync(imagesFile)) {
    fs.writeFileSync(imagesFile, JSON.stringify(createDefaultImages(), null, 2));
  }
}
...
function createUploadHandler(category) {
  ...
  return multer({
    ...
    fileFilter: (req, file, cb) => {
      ...
      if (allowedTypes.includes(mime) && allowedExtensions.includes(extension)) {
        cb(null, true);
      } else {
...
function removeOldFile(category, fileName) {
  if (!fileName) return;
  const targetFile = path.join(category === 'home' ? homeDir : galleryDir, fileName);
  if (fs.existsSync(targetFile)) fs.unlinkSync(targetFile);
}

router.get('/gallery', (req, res) => {
  const data = readImages();
  const gallery = normalizeItems(data.gallery).map((item) => ({ ...item, url: getPublicUrl(item.file, 'gallery') }));
  res.json({ success: true, data: gallery });
});

router.get('/home', (req, res) => {
  const data = readImages();
  const home = normalizeItems(data.home).map((item) => ({ ...item, url: getPublicUrl(item.file, 'home') }));
...
router.post('/gallery/upload', galleryUpload.single('image'), (req, res) => {
  try {
    ...
    const data = readImages();
    const nextId = Math.max(0, ...data.gallery.map((item) => Number(item.id) || 0)) + 1;
    const item = { id: nextId, file: req.file.filename, order: data.gallery.length + 1 };
    data.gallery.push(item);
    writeImages(data);
    res.status(201).json({ success: true, data: { ...item, url: getPublicUrl(item.file, 'gallery') } });
  } catch (error) {
...
router.post('/home/upload', homeUpload.single('image'), (req, res) => {
  try {
    if (!req.file) return res.status(400).json({ error: 'No file uploaded.' });
    const data = readImages();
    const nextId = Math.max(0, ...data.home.map((item) => Number(item.id) || 0)) + 1;
    const item = { id: nextId, file: req.file.filename, order: data.home.length + 1 };
    data.home.push(item);
    writeImages(data);
    res.status(201).json({ success: true, data: { ...item, url: getPublicUrl(item.file, 'home') } });
  } catch (error) {
...
router.put('/gallery/reorder', (req, res) => {
  try {
    ...
    const data = readImages();
    const reordered = normalizeItems(incoming).map((item, index) => ({ ...item, order: index + 1 }));
    data.gallery = reordered;
    writeImages(data);
    res.json({ success: true, data: reordered.map((item) => ({ ...item, url: getPublicUrl(item.file, 'gallery') })) });
  } catch (error) {
...
router.put('/home/reorder', (req, res) => {
  try {
    ...
    data.home = reordered;
    writeImages(data);
    res.json({ success: true, data: reordered.map((item) => ({ ...item, url: getPublicUrl(item.file, 'home') })) });
  } catch (error) {
...
router.delete('/gallery/:id', (req, res) => {
  try {
    const data = readImages();
    const item = data.gallery.find((image) => String(image.id) === String(req.params.id));
    if (!item) return res.status(404).json({ error: 'Gallery image not found.' });
    removeOldFile('gallery', item.file);
    data.gallery = normalizeItems(data.gallery.filter((image) => String(image.id) !== String(req.params.id))).map((image, index) => ({ ...image, order: index + 1 }));
    writeImages(data);
...
router.delete('/home/:id', (req, res) => {
  try {
    const data = readImages();
    const item = data.home.find((image) => String(image.id) === String(req.params.id));
    if (!item) return res.status(404).json({ error: 'Home image not found.' });
    removeOldFile('home', item.file);
    data.home = normalizeItems(data.home.filter((image) => String(image.id) !== String(req.params.id))).map((image, index) => ({ ...image, order: index + 1 }));
    writeImages(data);
...
router.put('/gallery/:id/replace', galleryUpload.single('image'), (req, res) => {
  try {
    ...
    writeImages(data);
    res.json({ success: true, data: { ...item, url: getPublicUrl(item.file, 'gallery') } });
  } catch (error) {
...
router.put('/home/:id/replace', homeUpload.single('image'), (req, res) => {
  try {
    ...
    item.file = req.file.filename;
    writeImages(data);
    res.json({ success: true, data: { ...item, url: getPublicUrl(item.file, 'home') } });
  } catch (error) {
...
router.use((error, req, res, next) => {
  if (error instanceof multer.MulterError) {
    if (error.code === 'LIMIT_FILE_SIZE') {
      return res.status(400).json({ error: 'Image exceeds the 5MB maximum size.' });
    }
  }
